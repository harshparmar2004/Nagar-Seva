...
import {
  ...
  Calendar, CheckSquare2, Info, Compass, AlertTriangle, ArrowRight, Activity, Map, Tag, LayoutGrid, List, ArrowUpDown, ChevronLeft, ChevronRight, Phone, CreditCard, Star, Construction, Bus, Sun, Trash2, Droplets, ArrowUpRight, BarChart3, ThumbsUp, PieChart as PieIcon, ChevronDown, ChevronUp, Award
} from 'lucide-react';
...
export default function AdminPortal({ activeSubTab, onOpenDPR, activeCountry, isSuperAdmin, onOpenAuth }) {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in pb-16">
      ...
      {/* SUB-TAB 3: DEDICATED BIGGER IN-PAGE PUBLISH PORTAL & RICH IN-PAGE PROJECT CARDS matching EXACT USER LAYOUT */}
      {(activeSubTab === 'admin-ranking' || activeSubTab === 'admin-dpr') && (
        <div className="space-y-6">
          ...
          {/* List of Rich Project Cards in Super Admin View matching EXACT User Reference Image 2 */}
          <div className="space-y-6">
            {projects.map((p, idx) => {
              const starsAvg = (4.4 + ((idx * 0.1) % 0.6)).toFixed(1);
              const totalVotes = p.community_upvotes || (1200 + idx * 850);
              const isExpanded = expandedProjectId === p.id;
              const budgetCrores = (p.estimated_budget_inr / 10000000).toFixed(1);

              return (
                <div
                  key={p.id}
                  className="bg-white border-2 border-purple-500 rounded-3xl p-6 space-y-5 shadow-sm hover:shadow-md transition-all relative overflow-hidden text-stone-900"
                >
                  {/* ── Top Row: Icon + Badges + Title + Budget Box + Expand Results Button ──────── */}
                  <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4 pb-2">
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="font-mono text-[11px] font-black text-stone-500 uppercase tracking-wider">{p.id}</span>
                        ...
                        {/* Main Title */}
                        <h3 className="text-lg sm:text-xl font-extrabold text-stone-900 leading-snug">{p.title}</h3>
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-xs text-stone-500 font-semibold pt-0.5">
                          ...
                          <span className="flex items-center gap-1">
                            <Building2 className="w-3.5 h-3.5 text-blue-600 shrink-0" /> {p.responsible_department || 'MPMRCL'}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center space-x-3 shrink-0">
                      ...
                      <div className="bg-emerald-50/80 border border-emerald-200 rounded-2xl px-6 py-3.5 text-center shrink-0">
                        ...
                        <p className="text-2xl font-black text-emerald-600 leading-none mt-0.5">₹{p.formatted_budget || `${budgetCrores} Cr`}</p>
                        <p className="text-[10px] text-stone-500 font-medium mt-1">{p.funding_scheme}</p>
                      </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div className="bg-blue-50/60 border border-blue-200/80 rounded-2xl p-3.5 text-center space-y-0.5">
                      ...
                      <p className="text-base font-extrabold text-stone-900">{p.target_beneficiaries?.toLocaleString('en-IN') || '500,000'}</p>
                    ...
                    <div className="bg-amber-50/60 border border-amber-200/80 rounded-2xl p-3.5 text-center space-y-0.5">
                      ...
                      <p className="text-base font-extrabold text-stone-900">{p.roi_score || '91'}/100</p>
                    ...
                    <div className="bg-purple-50/60 border border-purple-200/80 rounded-2xl p-3.5 text-center space-y-0.5">
                      ...
                      <p className="text-base font-extrabold text-stone-900">{totalVotes.toLocaleString()}</p>
                    ...
                    <div className="bg-emerald-50/60 border border-emerald-200/80 rounded-2xl p-3.5 text-center space-y-0.5">
                      ...
                      <p className="text-xs font-extrabold text-stone-900 leading-tight truncate">{p.responsible_ministry || 'MoHUA / MoRTH'}</p>
                  ...
                  {/* ── Bottom Row: Actions & Results Toggle ────────────── */}
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-3 border-t border-stone-100 text-xs font-bold">
                    ...
                    <div className="flex items-center gap-2.5">
                      <button
                        onClick={() => onOpenDPR(p)}
                        className="bg-stone-900 hover:bg-stone-800 text-white text-xs font-extrabold px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm"
                      >
                      ...
                      <button
                        onClick={() => {
                          if (p.scope_of_work && p.scope_of_work.length > 0) {
                            alert(`📋 Scope of Work for ${p.title}:\n\n${p.scope_of_work.map((s, i) => `${i + 1}. ${s}`).join('\n')}`);
                          } else {
                        ...
                        className="bg-stone-100 hover:bg-stone-200 text-stone-700 text-xs font-extrabold px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer border border-stone-200"
                      >
