...
export default function AnalyticsView() {
  ...
  // Dynamic Monthly/Yearly Data Generator based on selectedYear & selectedMonth
  const getMonthlyStats = () => {
    const is2026 = selectedYear === '2026';
    const is2025 = selectedYear === '2025';

    // Multipliers for realistic historical variation across years & months
    let yearMultiplier = is2026 ? 1.0 : is2025 ? 0.72 : 0.48;
    
    let monthMultiplier = 1.0;
    if (selectedMonth === 'JUN') monthMultiplier = 0.55;
    else if (selectedMonth === 'JUL') monthMultiplier = 0.85;
    else if (selectedMonth === 'AUG') monthMultiplier = 1.25;
    else if (selectedMonth === 'SEP') monthMultiplier = 0.95;
    else if (selectedMonth === 'OCT') monthMultiplier = 0.70;
    else if (selectedMonth === 'NOV') monthMultiplier = 0.50;
    else if (selectedMonth === 'DEC') monthMultiplier = 0.45;
    else if (selectedMonth === 'JAN') monthMultiplier = 0.40;
    else if (selectedMonth === 'FEB') monthMultiplier = 0.42;
    else if (selectedMonth === 'MAR') monthMultiplier = 0.46;
    else if (selectedMonth === 'APR') monthMultiplier = 0.50;
    else if (selectedMonth === 'MAY') monthMultiplier = 0.52;

    const baseTotal = Math.round((selectedMonth === 'ALL' ? (allComplaints.length || 850) : 450) * yearMultiplier * monthMultiplier);
    const resolved = Math.round(baseTotal * 0.88);
    const pending = Math.round(baseTotal * 0.10);
    const approved = Math.max(1, baseTotal - resolved - pending);
    const ppiScore = Math.min(98, (80 + (yearMultiplier * 5) + (monthMultiplier * 3))).toFixed(1);

    const categories = [
      { name: 'Sanitation & Drainage', count: Math.round(baseTotal * 0.48) },
      { name: 'Roads & Infrastructure', count: Math.round(baseTotal * 0.26) },
      { name: 'Water Supply', count: Math.round(baseTotal * 0.14) },
      { name: 'Electricity & Lighting', count: Math.round(baseTotal * 0.08) },
      { name: 'Solid Waste Management', count: Math.round(baseTotal * 0.04) }
    ];

    const trendLabelPrefix = selectedMonth === 'ALL' ? selectedYear : `${selectedMonth} ${selectedYear}`;
    const trend = [
      { month: `${trendLabelPrefix} W1`, requests: Math.round(baseTotal * 0.18), resolved: Math.round(baseTotal * 0.16) },
      { month: `${trendLabelPrefix} W2`, requests: Math.round(baseTotal * 0.24), resolved: Math.round(baseTotal * 0.22) },
      { month: `${trendLabelPrefix} W3`, requests: Math.round(baseTotal * 0.28), resolved: Math.round(baseTotal * 0.25) },
      { month: `${trendLabelPrefix} W4`, requests: Math.round(baseTotal * 0.30), resolved: Math.round(baseTotal * 0.27) },
    ];
    ...
    const mismatch = [
      { ward: 'Ward 14 (Rajendra Ngr)', demand: Math.min(99, Math.round(92 * monthMultiplier)), budget: is2026 ? 0 : 15 },
      { ward: 'Ward 15 (Silicon City)', demand: Math.min(99, Math.round(86 * monthMultiplier)), budget: is2026 ? 0 : 10 },
      { ward: 'Ward 8 (Banganga)', demand: Math.min(99, Math.round(80 * monthMultiplier)), budget: 25 },
      { ward: 'Ward 7 (Chandan Ngr)', demand: Math.min(99, Math.round(72 * monthMultiplier)), budget: 30 },
      { ward: 'Ward 2 (Vijay Nagar)', demand: Math.min(99, Math.round(35 * monthMultiplier)), budget: 90 },
      { ward: 'Ward 3 (Palasia)', demand: Math.min(99, Math.round(28 * monthMultiplier)), budget: 85 }
    ];

    const solvedVsRequested = [
      { month: `${selectedMonth === 'ALL' ? 'Q1' : selectedMonth} ${selectedYear}`, requested: Math.round(baseTotal * 0.3), solved: Math.round(baseTotal * 0.27), pending: Math.round(baseTotal * 0.03) },
      { month: `${selectedMonth === 'ALL' ? 'Q2' : selectedMonth} ${selectedYear}`, requested: Math.round(baseTotal * 0.6), solved: Math.round(baseTotal * 0.54), pending: Math.round(baseTotal * 0.06) },
      { month: `${selectedMonth === 'ALL' ? 'Current Cycle' : selectedMonth} ${selectedYear}`, requested: baseTotal, solved: resolved, pending: pending },
    ];
  ...
  if (loading) {
    return (
      <div className="max-w-7xl mx-auto py-16 flex flex-col items-center justify-center space-y-3 text-stone-500">
        ...
        <p className="text-xs font-bold">Loading Year & Month Data Fusion Analytics...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 animate-fade-in pb-16">
      ...
      {/* TOP HEADER WITH DYNAMIC YEAR & 12-MONTH SELECTOR FILTER BAR */}
      <div className="bg-white border border-stone-200 rounded-3xl p-5 space-y-4 shadow-sm">
        ...
        {/* YEAR & 12-MONTH RECORD CONTROL BAR */}
        <div className="bg-orange-50/70 border border-orange-200/80 rounded-2xl p-4 space-y-3 text-xs">
          
          {/* TOP STRIP: YEAR SELECTOR & TITLE */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-orange-200/60 font-bold">
            <div className="flex items-center space-x-2 text-stone-800">
              <Calendar className="w-4 h-4 text-orange-600 shrink-0" />
              <span className="uppercase tracking-wider text-[11px]">Filter Graphs by Year & Month:</span>
            </div>

            {/* YEAR SELECTOR BUTTONS */}
            <div className="flex items-center space-x-2">
              <span className="text-stone-500 text-[11px]">Select Year:</span>
              {YEARS_LIST.map((yr) => (
                <button
                  key={yr}
          ...
          {/* BOTTOM STRIP: 12 MONTHS INTERACTIVE PILL GRID */}
            ...
            {MONTHS_LIST.map((m) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl font-bold transition-all text-[11px] cursor-pointer ${
                  selectedMonth === m.id
                    ? 'bg-orange-600 text-white shadow-md shadow-orange-600/20 ring-2 ring-orange-400/30'
                    : 'bg-white text-stone-700 hover:bg-stone-100 border border-stone-200'
                }`}
              >
                {m.label}
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white border border-stone-200 rounded-3xl p-5 space-y-3 shadow-sm">
          <div className="flex items-center justify-between pb-2 border-b border-stone-100">
            ...
            <span className="bg-orange-100 text-orange-800 text-[10px] font-bold px-2.5 py-0.5 rounded-full border border-orange-200">
              {selectedMonth === 'ALL' ? 'All Months' : selectedMonth} {selectedYear} Breakdown
            </span>
          </div>
        ...
        {/* GRAPH 2: REQUEST SURGE TREND */}
        <div className="bg-white border border-stone-200 rounded-3xl p-5 space-y-3 shadow-sm">
          <div className="flex items-center justify-between pb-2 border-b border-stone-100">
            <h3 className="text-sm font-extrabold text-stone-900 flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-emerald-600" /> 2. Request Surge Trend ({selectedMonth === 'ALL' ? 'All Months' : selectedMonth} {selectedYear})
            </h3>
        ...
        <div className="bg-white border border-stone-200 rounded-3xl p-5 space-y-3 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs font-bold">
            <button
              onClick={() => { setSelectedYear('2026'); setSelectedMonth('JUN'); }}
              className={`p-3.5 rounded-2xl border text-left transition-all cursor-pointer ${
                selectedYear === '2026' && selectedMonth === 'JUN' ? 'bg-orange-100 border-orange-400 ring-2 ring-orange-400/20' : 'bg-stone-50 border-stone-200 hover:border-orange-300'
              }`}
            >
            ...
            <button
              onClick={() => { setSelectedYear('2026'); setSelectedMonth('JUL'); }}
              className={`p-3.5 rounded-2xl border text-left transition-all cursor-pointer ${
                selectedYear === '2026' && selectedMonth === 'JUL' ? 'bg-orange-100 border-orange-400 ring-2 ring-orange-400/20' : 'bg-stone-50 border-stone-200 hover:border-orange-300'
              }`}
            >
            ...
            <button
              onClick={() => { setSelectedYear('2026'); setSelectedMonth('AUG'); }}
              className={`p-3.5 rounded-2xl border text-left transition-all cursor-pointer ${
                selectedYear === '2026' && selectedMonth === 'AUG' ? 'bg-orange-100 border-orange-400 ring-2 ring-orange-400/20' : 'bg-orange-50/70 border-orange-200 hover:border-orange-300'
              }`}
            >
